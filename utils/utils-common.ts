...
import { advancedDayjs } from "./date-utils";
import { sortObject } from "./js-utils";
import type { OrderPaymentPlan } from "screens/Order/OrderTypes";
import Layout from "constants/Layout";

export const findIdentityKyc = (kycs: any[]) => {
  return kycs.find(
    (kyc) => kyc.kyc_type === "sg_identity" || kyc.kyc_type === "my_identity"
  );
};

export const findPaymentMethodKyc = (kycs: any[]) => {
  return kycs.find(
    (kyc) =>
      kyc.kyc_type === "sg_payment_method" ||
      kyc.kyc_type === "my_payment_method"
  );
};

export const logoutAlert = (cb: () => void) => {
...
export interface KycsObjectSG {
  sg_identity: string | null;
  sg_identity_validity: string | null;
  sg_credit_bureau_report: string | null;
  sg_credit_bureau_report_validity: string | null;
  sg_payment_method: string | null;
  sg_payment_method_validity: string | null;
}

export interface KycsObjectMY {
  my_identity: string | null;
  my_identity_validity: string | null;
  my_credit_bureau_report: string | null;
  my_credit_bureau_report_validity: string | null;
  my_payment_method: string | null;
  my_payment_method_validity: string | null;
}

export interface KycsObject extends KycsObjectSG, KycsObjectMY {}

export const getKycsObject = (responseCredits: any): KycsObject => {
  return (
    responseCredits?.kycs?.reduce((acc: any, cur: any) => {
      const { kyc_type, status, expired_at } = cur;
      if (!kyc_type) return acc;
...
export const getKycsObjectKey = (
  countryCode: "sg" | "my",
  key:
    | "identity"
    | "identity_validity"
    | "credit_bureau_report"
    | "credit_bureau_report_validity"
    | "payment_method"
    | "payment_method_validity",
  kycsObject: KycsObject
): string => {
  if (countryCode === "sg") {
    return (kycsObject as any)?.[`sg_${key}` as any] as string;
  } else {
    return (kycsObject as any)?.[`my_${key}` as any] as string;
  }
};

// "sg_identity": "data_available",
// "sg_identity_validity": "2022-01-27T10:35:50.338674+08:00",
// "sg_credit_bureau_report": "incomplete",
// "sg_credit_bureau_report_validity": null,
// "sg_payment_method": "incomplete",
// "sg_payment_method_validity": null

export const getApprovedCredit = (responseCredits?: any): any => {
  const { amount, assessment } = responseCredits ?? {};
  const { completed_at } = assessment ?? {};

  if (amount && completed_at) {
    return {
      amount,
...
interface KycsBtnData {
  btnVariant: null | string;
  btnProps: { onPress?: () => {}; disabled?: boolean };
  textProps?: ViewProps & { color?: string };
}

export const getKycsBtnData = (key: string, obj: KycsObject): KycsBtnData => {
  const status = (obj as any)[key];
  const validityKey = `${key}_validity`;
  const validityDate = (obj as any)[validityKey];
  let textProps = { children: "" } as KycsBtnData["textProps"],
    btnVariant: KycsBtnData["btnVariant"] = "orangeStart";

  if (status === "data_available" && validityDate) {
    btnVariant = "greenCheck";
    textProps = {
      children: `Valid until ${format(new Date(validityDate), "dd MMM yyyy")}`,
    };
  } else if (status === "incomplete" || status === "data_error") {
    textProps = { children: `Not complete` };
  } else if (status === "expired") {
    textProps = {
      color: theme.colors.buttons.darkTangerine,
      children: `Validity Expired`,
    };
  } else if (
    status === "reviewing_for_credit" ||
    status === "created" ||
...
export const checkAllKycsCompleted = (kycs: any[]) => {
  return kycs.every((kyc) => {
    if (kyc.kyc_type && kyc.kyc_type === "sg_home_ownership") {
      // do not take home ownership into account
      return true;
    }
    return (
      kyc.status === "data_available" || kyc.status === "reviewing_for_credit"
    );
  });
};

// from npm install creditcardutils
export const getCardIcon = (type: "visa" | "mastercard" | "mc") => {
  let cardTypeIcon: any = {};
...
export const groupRepaymentsIntoMonths = (repayments: any) => {
  const results: any = {};
...
export const calculateTodaysPayment = (
  ...
  const sum =
    ...
    parseFloat(instalment_amount) +
    parseFloat(instalment_processing_fee_amount);

  return sum;
};

export const secureCardNumber = (cardNo: any) => {
...
export const checkForInsufficientCredit = (
  paymentPlans: OrderPaymentPlan[]
) => {
  const insufficientCredit = paymentPlans.some(
    (plan) => plan.insufficient_credit === true
  );
  const minSufficientCreditAmounts = paymentPlans
    .filter((plan) => plan.insufficient_credit === true)
    .map((plan) => plan.sufficient_credit_amount);
  const minSufficientCreditAmount =
    minSufficientCreditAmounts.length > 0
      ? Math.min(...(minSufficientCreditAmounts as any)).toFixed(2)
      : "0.00";
  return { insufficientCredit, minSufficientCreditAmount };
};

export const findMinDownpayment = (paymentPlans: OrderPaymentPlan[]) => {
  let minDownpayment = Infinity;
  paymentPlans.map((plan) => {
    const { downpayment_amount } = plan;
    return (minDownpayment = Math.min(
      parseFloat(downpayment_amount),
      minDownpayment
    ));
  });

  return minDownpayment.toString();
};

interface GetGenericSkeletonConfigProps {
  layoutScreenWidth: number;
  layoutScreenHeight: number;
}

export const getGenericSkeletonConfig = (
  allProps?: GetGenericSkeletonConfigProps
) => {
  let { layoutScreenWidth, layoutScreenHeight } = allProps ?? {};
  const layoutScreen = Layout.screen;
  layoutScreenWidth = !layoutScreenWidth
    ? layoutScreen.width
    : layoutScreenWidth;
  layoutScreenHeight = !layoutScreenHeight
    ? layoutScreen.height
    : layoutScreenHeight;
  ...
  return [
    ...
    },
    {
      ...
      marginLeft: 0,
      height: layoutScreenHeight * 0.03,
    },
    {
      key: "content3",
      marginTop: layoutScreenHeight * 0.02,
      ...
      marginLeft: 0,
      height: layoutScreenHeight * 0.02,
    },
  ];
};

export const groupNotificationsIntoDays = (notifications: any) => {
  let results: any = {};
