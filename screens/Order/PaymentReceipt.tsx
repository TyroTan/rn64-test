...
const RenderPaymentDetailsInstalment = (
  props: Pick<ResponseConfirmOrder["data"], "order" | "repayment">
) => {
...
const RenderPaymentDetailsPayLater = (
  props: Pick<ResponseConfirmOrder["data"], "order" | "repayment">
) => {
  // const { formatAsCurrency } = globalObjectState.getLibrary();
  const { order, repayment } = props;
...
const PaymentReceipt: React.FC<any> = (props: any) => {
  const { route, navigation, setHeaderProgress } = props;
  const orderConfirm: ResponseConfirmOrder =
    route?.params?.responseOrderConfirm;

  const { order, repayment } = orderConfirm.data;
  ...
  // * Handlers - onpress
  const onReturn = () => {
    // setFromOrderPaymentReceipt("goToTransaction");
    globalObjectLastActionState.resetBeforeUse();
    globalObjectLastActionState.set("fromPaymentReceipt", {
      action: "fromPaymentReceipt",
      data: {
        orderCode: order.code,
      },
    });
    navigation.navigate(CreditRoutes.CreditInitial);
  };
    ...
    >
      ...
      >
        ...
        >
          ...
          <RenderPaymentDetails order={order} repayment={repayment} />
          <ButtonText
            onPress={onReturn}
